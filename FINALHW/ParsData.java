...
public class ParsData {
    public HashMap<String, Object> parsInputData() {
        ...
        for (String i : data) {       // символ пола
            ...
            } else if (i.matches("\\d{1,2}\\.\\d{1,2}\\.\\d{4}")) { // корректность ввода даты
            ...
            } else if (i.matches("[0-9]+")) { // ввод номера телефона
                dataDic.put("tel", i);
            } else if (i.matches("[A-Za-z]+")) { // ввод ФИО
                sb.append(i + " ");
            } else {
                try {
                    throw new DataException();
        ...
        String[] fullName = String.valueOf(sb).split(" "); // объединение ФИО
